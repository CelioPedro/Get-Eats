...
class CarrinhoManager {
    constructor() {
        ...
        this.endereco = null; // Endere?o de entrega
    ...
    calcularDesconto() {
        ...
        // Cupons dispon?veis: cupom5 (R$5,00), cupom10 (R$10,00), cupom12 (R$12,00)
        const cupons = {
            'cupom5': 5.00,
            'cupom10': 10.00,
            'cupom12': 12.00
    ...
    calcularTotal() {
        ...
        return Math.max(total, 0); // N?o permitir total negativo
    }

    // Aplicar cupom
    aplicarCupom(codigo) {
        // Lista de cupons v?lidos (poderia vir de uma API)
    ...
    salvarCarrinho() {
        ...
        if (usuarioLogado) {
            ...
            // Usar chave gen?rica para usu?rios n?o logados
    ...
    carregarCarrinho() {
        ...
        const usuarioLogado = authManager.obterUsuarioLogado();
        if (!usuarioLogado) {
            // Se n?o estiver logado, usar carrinho gen?rico (compatibilidade)
        ...
        if (dadosSalvos) {
            ...
            this.carrinho = dados.itens || [];
            this.cupomAplicado = dados.cupom || null;
            this.endereco = dados.endereco || usuarioLogado.endereco; // Usar endere?o do perfil se n?o houver no carrinho
        } else {
            // Primeiro acesso: carregar endere?o do perfil
    ...
    limparCarrinho() {
        this.carrinho = [];
        this.cupomAplicado = null;
        // N?o limpar endere?o, manter o ?ltimo usado
        this.salvarCarrinho();
        this.atualizarInterface();
    }
    ...
    // Atualizar interface (ser? implementado nas p?ginas espec?ficas)
    atualizarInterface() {
        // Este m?todo ser? sobrescrito nas p?ginas que usam o carrinho
...
// Inst?ncia global do gerenciador do carrinho
const carrinhoManager = new CarrinhoManager();

// Fun??es de utilidade para integra??o com p?ginas HTML
